app.service('AuthService', ['$q', '$timeout', '$http','$rootScope','$cookieStore',
  function($q, $timeout, $http,$rootScope,$cookieStore) {
    var uerLogin = {};
    var user = null;
    var users = [];
    ...
    function register(user) {
      ...
      $http.post('api/user/register', {
        ...
        .then(function(response) {
          ...
          if (response.status === 200 && response.status) {
              $cookieStore.put('currentUser', response.data.account);
            deferred.resolve();
          } else {
            deferred.reject();
          }
